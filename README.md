...
本项目使用 Vue 3 + Vite 开发，接口代理配置如下：

```typescript
// vite.config.ts
proxy: {
  '/api': {
    target: 'http://localhost:3456',  // 替换为实际后端接口地址
    changeOrigin: true,
  }
}
```
...
以下功能模块需要完成对应接口对接才能正常使用：

#### 5.2.1 认证模块

| 功能 | 接口路径 | 方法 | 状态 |
|------|----------|------|------|
| 用户登录 | `/api/auth/login` | POST | ✅ 已定义 |
| 获取公司列表 | - | - | ⚠️ 模拟数据 |

**说明**：公司列表目前使用模拟数据，需后续开发落实真实接口。

#### 5.2.2 工单管理模块

| 功能 | 接口路径 | 方法 | 状态 |
|------|----------|------|------|
| 获取工单列表 | `/api/work-orders` | GET | ✅ 已定义 |
| 启用工单 | `/api/work-orders/{id}/enable` | PUT | ✅ 已定义 |
| 关闭工单 | `/api/work-orders/{id}/close` | PUT | ✅ 已定义 |
| 创建工单 | `/api/work-orders` | POST | ✅ 已定义 |
| 获取工单树 | `/api/work-orders/tree` | GET | ✅ 已定义 |

#### 5.2.3 任务管理模块

| 功能 | 接口路径 | 方法 | 状态 |
|------|----------|------|------|
| 获取任务列表 | `/api/tasks` | GET | ✅ 已定义 |
| 获取任务详情 | `/api/tasks/{id}` | GET | ✅ 已定义 |
| 创建任务 | `/api/tasks` | POST | ✅ 已定义 |
| 更新任务 | `/api/tasks/{id}` | PUT | ✅ 已定义 |
| 更新任务状态 | `/api/tasks/{id}/status` | PUT | ✅ 已定义 |
| 删除任务 | `/api/tasks/{id}` | DELETE | ✅ 已定义 |
| 恢复任务 | `/api/tasks/{id}/restore` | PUT | ✅ 已定义 |

#### 5.2.4 测量数据模块

| 功能 | 接口路径 | 方法 | 状态 |
|------|----------|------|------|
| 获取任务测量数据 | `/api/measurement/task/{taskId}` | GET | ✅ 已定义 |
| 创建测量数据 | `/api/measurement/task/{taskId}` | POST | ✅ 已定义 |
| 批量导入测量数据 | `/api/measurement/task/{taskId}/batch` | POST | ✅ 已定义 |
| 更新测量数据 | `/api/measurement/{id}` | PUT | ✅ 已定义 |
| 删除测量数据 | `/api/measurement/{id}` | DELETE | ✅ 已定义 |

#### 5.2.5 控制图分析模块

| 功能 | 接口路径 | 方法 | 状态 |
|------|----------|------|------|
| 获取任务控制图数据 | `/api/control-chart/task/{taskId}` | GET | ✅ 已定义 |
| 获取工单树（含任务） | `/api/work-orders/tree` | GET | ✅ 已定义 |

#### 5.2.6 过程能力分析模块

| 功能 | 接口路径 | 方法 | 状态 |
|------|----------|------|------|
| 获取任务过程能力数据 | `/api/capability/task/{taskId}` | GET | ✅ 已定义 |
